...
class Net:
    def __init__(self, args):
        ...
        self.t_feat = np.load(os.path.join(self.data_path, 'text_feat.npy'),allow_pickle=True)
    ...
    def run(self):
        ...
        for epoch in range(self.num_epoch):
            ...
            epoch = {'loss':loss_avg, 'loss_BPR': loss_BPR_avg, 'loss_Price': loss_Price_avg, 
                     'recall': recall, 'ndcg': ndcg, # top50으로 비교
                     'recall_test': recall_test, 'ndcg_test': ndcg_test, # top50으로 비교
...
if __name__ == '__main__':
    ...
    mmmo = Net(args)
    wandb.init(project=args.project, name=f'{args.collection}__{args.model_name}_{args.l_r}_{args.dim_x}_{args.loss_alpha}_{args.batch_size}_{args.reg_parm}_{args.hop}', config={k: v for k, v in args._get_kwargs()})
    mmmo.run()
    wandb.finish()
